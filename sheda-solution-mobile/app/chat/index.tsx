...
const Chat = () => {
  ...
  return (
    <SafeAreaView className="h-screen w-full mx-auto max-w-2xl">
      <View className="flex-row justify-between items-center px-5 p-3">
        <View className="flex-row gap-5 items-center">
          ...
          <View className="flex-row gap-2">
            <View className="items-center justify-center rounded-full overflow-hidden w-[38] h-[38] ">
              <Image source={PROFILE} style={{ width: 38, height: 38 }} />
            </View>
            <View>
              <InterBold className="text-sm/[150%]">Seller's Name</InterBold>
              <InterRegular className="text-secondaryText text">
                Active now
              </InterRegular>
            </View>
          </View>
      ...
      <View>
        <View className="px-5 bg-[#00000008] flex-row gap-3 py-[10] border-t border-borderColor">
          <Image source={APT} style={{ width: 35, height: 35 }} />
          <View>
            <InterRegular className="text-sm/[150%]">
              The house title
            </InterRegular>
            <InterBold>N320k/yr</InterBold>
          </View>
        </View>
        {toggleMenu && (
          <View className="absolute right-0 top-0 bg-slate-50 z-10">
            <InboxMenu />
          </View>
        )}
      </View>
