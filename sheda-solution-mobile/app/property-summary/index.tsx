...
const PropertySummary = () => {
  ...
  const { paymentInfo, getPaymentInfo } = useApi();
  ...
  useEffect(() => {
    if (propertyId) {
      getPaymentInfo(Number(propertyId));
    }
  }, [propertyId, getPaymentInfo]);
        ...
        <View className="border-t border-dotted mt-8">
          <View className="flex-row justify-between mt-10">
            ...
            <InterRegular className="text-xs/[150%]">
              {paymentInfo?.payment_method || "Not selected"}
            </InterRegular>
          </View>
          <View className="flex-row justify-between my-3">
            <InterMedium className="text-xs/[150%]">Charge</InterMedium>
            <InterSemiBold className="text-xs/[150%]">
              {paymentInfo?.amount ? `N${paymentInfo.amount}` : "N/A"}
            </InterSemiBold>
          </View>
          <View className="flex-row justify-between">
            <InterMedium className="text-xs/[150%]">Rate</InterMedium>
            <InterRegular className="text-xs/[150%]">
              {paymentInfo?.exchange_rate || "1.00"} NGN/USD
            </InterRegular>
          </View>
        </View>
      </ScrollView>
