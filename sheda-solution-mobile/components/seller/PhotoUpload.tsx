...
const PhotoUpload: React.FC<PhotoUploadProps> = ({ onPhotosChange, photos }) => {
  ...
  const renderPhotoSlot = (index: number) => {
    ...
    return (
      ...
      >
        {hasPhoto ? (
          <Image source={{ uri: hasPhoto }} className="w-full h-full rounded-lg" resizeMode="cover" />
        ) : (
  ...
  return (
    <View>
      <InterRegular className="text-base mb-4">Add at least 4 photos</InterRegular>
      <View className="flex-row flex-wrap justify-between">{[0, 1, 2, 3].map(renderPhotoSlot)}</View>

      {photos.length >= 4 && (
        <TouchableOpacity onPress={pickImage} className="flex-row items-center mt-4">
